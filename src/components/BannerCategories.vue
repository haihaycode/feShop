...
<template>
  <voucher></voucher>
  <div class=" grid md:grid-cols-3 grid-cols-1 border my-4 rounded-sm"
    style="background-image: url(https://img.freepik.com/free-vector/red-diagonal-geometric-shape-with-halftone-detailed-background_1409-2103.jpg); background-repeat: no-repeat;background-size: 100% 100%;">

    <div class="col-span-1  md:block lg:block  overflow-x-hidden md:ml-4 ml-0">
      <div class=" transition-transform duration-500 ease-in-out carousel-container rounded-sm  md:h-full "
        :style="{ transform: `translateX(-${newsIndex * 100}%)` }">
        <div v-for="n in list" :key="n.id" class="relative w-full flex-shrink-0  md:h-full">
          <img :src="n.image" alt="Product Image" class="w-full h-full md:object-contain object-contain rounded-none">
        </div>
    ...
    <div class="col-span-2 relative">
      <div class=" p-4">
        ...
        <button @click="scrollLeft"
          class=" md:block lg:block absolute top-1/2 left-0 md:left-[-10px] transform -translate-y-1/2 bg-white shadow p-2 rounded-full">
          &lt;
        </button>
        <div ref="scrollContainer" class="custom-scrollbarWhite overflow-x-auto hide-scrollbar">
          <ul class="grid  md:grid-rows-3 grid-rows-2 grid-flow-col gap-1 md:gap-4 lg:gap-4 items-center">
            <li v-for="(a, index) in array" :key="index" class="w-[150px] text-center">
              <img :src="a.logo" :alt="a.ten"
                class="w-full h-[75px] md:h-[80px]  lg:h-[150px] bg-white object-contain mx-auto rounded-md">
            </li>
          </ul>
        </div>
        ...
        <button @click="scrollRight"
          class=" md:block lg:block absolute top-1/2 right-0 md:right-[-10px] transform -translate-y-1/2 bg-white shadow p-2 rounded-full">
          &gt;
        </button>
      </div>
    </div>
